...
const GetProductsByCategory = require('../usescases/GetProductsByCategory');
...
class CategoryController {
    ...
    createCategory(request, h) {
        const { id, code, name, image } = request.payload;
        const useCase = new CreateCategory(this.repository);
        useCase.setCategory(new Category(id, code, name, image));
    ...
    async getProductsByCategory(request, h) {
        ...
        const useCase = new GetProductsByCategory(this.repository);
        useCase.setCategoryId(categoryId);
        const response = h.response({ "data": await useCase.execute() }).code(200).type('application/json');
        response.header("Authorization", request.headers.authorization);
        return response;
    }
